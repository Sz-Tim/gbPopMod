...
##---
  ...
  set.seed(225)
  n.sim <- 10
  g.p <- set_g_p(tmax=100, lc.r=30, lc.c=30, n.cores=1)
...
sb.mn <- apply(log(out.sb+1), 1:2, mean)
sb.sd <- apply(log(out.sb+1), 1:2, sd)
sb.pP <- apply(out.sb > 0, 1:2, mean)
ad.mn <- apply(out.ad, 1:2, mean)
ad.sd <- apply(out.ad, 1:2, sd)
ad.pP <- apply(out.ad > 0, 1:2, mean)
N.out <- cbind(lc.df, ad.mn) %>% as.tibble %>%
  gather(year, N.adult, (1:ncol(ad.mn)) + ncol(lc.df)) %>%
  mutate(sd.ad=c(ad.sd),
         pP.ad=c(ad.pP),
         N.sb=c(sb.mn),
         sd.sb=c(sb.sd),
         pP.sb=c(sb.pP),
