...
##---
  ...
  set.seed(225)
  n.sim <- 2
  g.p <- set_g_p(tmax=100, lc.r=50, lc.c=50, n.cores=1)
...
```{r runSim}
out.N <- array(dim=c(ngrid, g.p$tmax+1, max(g.p$age.f), n.sim))
...
out <- list(N=apply(out.N, 1:3, mean),
            N.sb=apply(out.sb, 1:2, mean))
```
