...
foreach(j=1:length(p.seq), .combine=rbind) %dopar% {
  ...
  cell.j <- cbind(lc.df, ad.mn) %>% as.tibble %>%
    ...
    mutate(sd.ad=c(apply(ad.j, 1:2, sd)),
           ...
           N.less5=c(ad.mn > 0))
  cell.j$N.less5 <- cell.j$N.less5 + c(ad.mn > 5)
  cell.j$year <- str_pad(cell.j$year, 3, "left", "0")
  cell.j$p <- p
  cell.j$p.j <- p.seq[j]
  ## landscape summaries
  occ.s.ad <- apply(ad.j[lc.df$inbd,,]>0, 2:3, mean)
  occ.s.sb <- apply(sb.j[lc.df$inbd,,]>0, 2:3, mean)
  less5.s <- apply(ad.j[lc.df$inbd,,]>0 & ad.j[lc.df$inbd,,]<=5, 2:3, mean)
  K.ag <- round(as.matrix(lc.df[,4:9]) %*% g.p$K)
  K.s <- apply(ad.j[lc.df$inbd,,]==K.ag[lc.df$inbd,], 2:3, mean)/occ.s.ad
  grid.j <- tibble(year=unique(cell.j$year),
                   p=p,
                   p.j=p.seq[j],
                   prOcc.ad.mn=apply(occ.s.ad, 1, mean),
                   prOcc.ad.sd=apply(occ.s.ad, 1, sd),
                   prOcc.sb.mn=apply(occ.s.sb, 1, mean),
                   prOcc.sb.sd=apply(occ.s.sb, 1, sd),
                   prLess5.mn=apply(less5.s, 1, mean),
                   prLess5.sd=apply(less5.s, 1, sd),
                   K.Occ.mn=apply(K.s, 1, mean),
                   K.Occ.sd=apply(K.s, 1, sd))
  ...
  make_plots_final_t(parSet.wd[j], g.p,  
                     filter(cell.j, year==max(cell.j$year)), p.j)
...
grid.sum <- map_df(parSet.wd, ~(read_csv(paste0(., "grid_j.csv")))) %>%
  mutate(p.j=as.factor(p.j), year=as.numeric(year))
make_plots_gridSummary(par.wd, grid.sum)
```
